...
const OrderInfo: FC<IOrderInfo> = ({ modal = false }) => {
  ...
  const ingredientsCounter: { count: any } = ingredientsOrder?.reduce(
    (prev: { count: any }, curr) => {
      const id = curr?._id;
    ...
    <>
      {!!order && (
        <div className={styleOrderInfo.container}>
          ...
          <div className="text text_type_main-medium pt-5 pb-3">
            {order.name}
          </div>
          <div
          ...
          <div className={`${styleOrderInfo.items} pr-6`}>
            {uniqIngredientsOrder?.map((ingredient) => (
              <div
                key={ingredient?._id}
                className={`${styleOrderInfo.item} pb-4`}
              >
